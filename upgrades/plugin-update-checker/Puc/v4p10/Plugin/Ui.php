	...
	class Puc_v4p10_Plugin_Ui {
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = sprintf(_x('Das %s Plugin ist auf dem neusten Stand.', 'the plugin title', 'plugin-update-checker'), $title);
				} else if ( $status == 'update_available' ) {
					$message = sprintf(_x('Es ist eine neue Version des %s Plugin verfügbar.', 'the plugin title', 'plugin-update-checker'), $title);
				} else if ( $status === 'error' ) {
					$message = sprintf(_x('Es kann nicht herausgefunden werden, ob ein Update für %s verfügbar ist.', 'the plugin title', 'plugin-update-checker'), $title);
